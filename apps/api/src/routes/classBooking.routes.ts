...
export default async function classBookingRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/api/v1/reservations', async (req, reply) => {
  ...
  fastify.delete('/api/v1/reservations/:id', async (req, reply) => {
    if (!(await gate(req, reply))) return;
    const { user } = req as AuthedRequest;
    return reply.send(await BookingSvc.cancel(user.orgId, user.sub, (req.params as { id: string }).id));
  });

  fastify.post('/api/v1/reservations/:id/check-in', async (req, reply) => {
    if (!(await gate(req, reply))) return;
    const { user } = req as AuthedRequest;
  ...
  fastify.post('/api/v1/reservations/:id/no-show', async (req, reply) => {
    if (!(await gate(req, reply))) return;
    const { user } = req as AuthedRequest;
    return reply.send({ reservation: await BookingSvc.markNoShow(user.orgId, user.sub, (req.params as { id: string }).id) });
  });

  fastify.get('/api/v1/sessions/:id/roster', async (req, reply) => {
    if (!(await gate(req, reply))) return;
    const { user } = req as AuthedRequest;
  ...
  // Waitlist
  fastify.get('/api/v1/sessions/:id/waitlist', async (req, reply) => {
    if (!(await gate(req, reply))) return;
    const { user } = req as AuthedRequest;
    return reply.send({ waitlist: await BookingSvc.listWaitlist(user.orgId, (req.params as { id: string }).id) });
  });
  fastify.post('/api/v1/sessions/:id/waitlist', async (req, reply) => {
    if (!(await gate(req, reply))) return;
    const { user } = req as AuthedRequest;
  ...
  fastify.post('/api/v1/waitlist/:id/promote', async (req, reply) => {
    if (!(await gate(req, reply))) return;
    const { user } = req as AuthedRequest;
